...
export default function FreelancerListing() {
    ...
    return (
        <div className="container mx-auto px-4 py-8 bg-white">
            <h1 className="text-2xl font-medium mb-6">Explore some of GIGFLARE Freelancers</h1>
                    ...
                    <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                        <div className="p-4">
                            <div className="flex items-start space-x-4">
                                <img src={freelancer.profile} alt={freelancer.firstName} className="w-16 h-16 rounded-full" />
                                <div>
                                    <h2 className="text-lg font-semibold">{freelancer.firstName} {freelancer.lastName}</h2>
                                    <p className="text-sm text-gray-600">{freelancer.language}</p>
                                    <p className="text-lg font-bold mt-2">RS:1000/hr</p>
                                </div>
                            </div>
                            <div className="mt-4">
                                {freelancer.skills.map((skill: any, skillIndex: any) => (
                                    <span key={skillIndex} className="inline-block bg-gray-100 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
                                        {skill}
                                    </span>
                                ))}
                            </div>
